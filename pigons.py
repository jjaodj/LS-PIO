...
class SigmoidalPigeon:
    ...
    def __init__(self, random=False):
        ...
        self.tpr = .0
        self.fpr = .0
        self.iforest_score = None
        self.lof_score = None
        self.one_class_svm_score = None
    ...
    def evaluate_with_lof(self, lof_model):
        X = [self.x()]
        self.lof_score = -lof_model.negative_outlier_factor_[0]

    def evaluate_with_isolation_forest(self, iforest_model):
        X = [self.x()]
        self.iforest_score = -iforest_model.score_samples(X)[0]

    def evaluate_with_one_class_svm(self, svm_model):
        X = [self.x()]
        self.one_class_svm_score = -svm_model.score_samples(X)[0]
